...
function calculateAge(){
    let dateOfBirth = new Date(inputDate.value);
    ...
    if(d2 >= d1){
        ...
        m3--;
        d3 = getFullDayInMonth(y1, m1) + d2 - d1;
...
showAge.innerHTML = `You are <span>${y3}</span> years, <span>${m3}</span> monhts and <span>${d3}</span> days old`;
